...
11. **Kewajiban Berkas `types.ts` Terpisah pada Submodul Frontend:**
   - DILARANG menuliskan tipe/interface TypeScript secara inline di dalam `Index.tsx` maupun berkas komponen di bawah `components/`.
   ...
   - Seluruh definisi tipe TypeScript submodul WAJIB diletakkan pada berkas tunggal `types.ts` di tingkat utama folder submodul tersebut (contoh: `resources/js/pages/Console/UserManagement/types.ts`).
   - Generator CLI `php artisan make:module` WAJIB otomatis membuat berkas `types.ts` ini pada saat pembuatan scaffold submodul baru.
