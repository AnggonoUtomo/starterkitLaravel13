        ...
        <div className="space-y-3 pt-1.5">
            {grouped.map((group) => (
                <div key={group.module} className="space-y-1.5">
                    <div className="border-b border-border/50 pb-1 text-[11.5px] font-bold tracking-wider text-muted-foreground/90 uppercase">
                        Modul: {group.module}
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                        {group.permissions.map((p) => (
                            <div
                                key={p}
                                className="flex items-center gap-2.5 rounded-md border border-border bg-background px-3 py-1.5 font-mono text-xs font-medium text-foreground shadow-2xs"
                            >
                                <IconComponent
        ...
        <div className="mt-4 space-y-4 text-xs">
                ...
                <div className="grid gap-2 rounded-lg border border-border bg-muted/20 p-3">
                    {availableRoles.map((roleName) => {
                        ...
                        return (
                            <label
                                key={roleName}
                                className="flex cursor-pointer items-center gap-2.5 text-xs font-semibold text-foreground"
                            >
                                <input
                                    type="checkbox"
                                    checked={isChecked}
            ...
            {formData.roles.length > 0 && (
                <div>
                    ...
                    <div className="max-h-60 space-y-3 overflow-y-auto rounded-lg border border-border bg-muted/10 p-3">
                        {formData.roles.map((roleName) => {
                            const roleObj = rolesWithPermissions.find(
                                (r) => r.name === roleName,
                            );
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center justify-between border-b border-border/60 pb-1.5 text-xs font-bold text-foreground capitalize">
            ...
            <div>
                ...
                <div className="max-h-64 space-y-3 overflow-y-auto rounded-lg border border-border bg-muted/10 p-3">
                    {permissionGroups.map((group) => (
                        <div
                            key={group.module}
                            className="space-y-2 rounded-lg border border-border bg-background p-3 shadow-2xs"
                        >
                            <div className="border-b border-border/60 pb-1 text-[11.5px] font-bold tracking-wider text-muted-foreground uppercase">
                                Kategori: {group.module}
                            </div>
                            <div className="flex flex-col gap-1.5">
                                {group.permissions.map((perm) => {
                                    const isInherited =
                                        inheritedPermissions.has(perm.name);
                                    const isDirectChecked =
                                    ...
                                    return (
                                        <label
                                            key={perm.id}
                                            className={`flex cursor-pointer items-center justify-between gap-2.5 rounded-md border px-3 py-1.5 text-xs transition ${
                                                isInherited
                                                    ? 'border-indigo-500/20 bg-indigo-500/5 text-muted-foreground opacity-80'
                                                    : isDirectChecked
                                                      ? 'border-emerald-500/40 bg-emerald-500/10 font-medium text-emerald-600'
                                        ...
                                        >
                                            <div className="flex min-w-0 items-center gap-2.5">
                                                <input
                                                    ...
                                                    className="size-4 cursor-pointer rounded border-border text-emerald-600 focus:ring-emerald-500 disabled:opacity-60"
                                                />
            ...
            <div className="flex justify-end gap-2 pt-2">
                <button
                    ...
                    className="cursor-pointer px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
                >
                    Batal
                </button>
                <button
                    type="submit"
                    disabled={isProcessing}
                    className={`cursor-pointer rounded-lg px-4 py-2 text-xs font-semibold text-white shadow-2xs transition disabled:cursor-not-allowed disabled:opacity-50 ${
                        isEditMode
    ...
    if (mode === 'create') {
        return (
            <div className="rounded-xl border border-border bg-card p-5 shadow-xs">
                ...
                <form onSubmit={onSubmitCreate}>{renderFormFields(false)}</form>
            </div>
        );
    }
    ...
    if (mode === 'edit' && selectedUser) {
        return (
            <div className="rounded-xl border border-border bg-card p-5 shadow-xs">
                ...
                <form onSubmit={onSubmitEdit}>{renderFormFields(true)}</form>
            </div>
        );
    }
